...
const getChannelIcon = (video_data) => {
  ...
  loading.innerHTML =
    '<div class="d-flex align-items-center justify-content-center mt-5"><div class="spinner-border"></div></div>';
...
filterButtons.forEach((btn) =>
  btn.addEventListener("click", async (e) => {
    ...
    const fetchURL = await fetch(
      `${keyword_http}part=snippet&maxResults=20&q=${filterValue}&key=${API_KEY}`
    );
    ...
    videoCardContainer.classList =
      "videoCardContainer row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 g-4 mt-3";
...
function filterFunc() {
  ...
  filterButtons.forEach((btn) => {
    if (btn.innerText.toLowerCase() !== data) {
      ...
      btn.classList = "search-filter bg-danger";
    }
  });
}
...
async function getMostPopularVideos() {
  try {
    ...
    const res = await fetch(
      `${video_http}part=snippet&chart=mostPopular&maxResults=20&regionCode=IN&key=${API_KEY}`
    );
...
formObj.addEventListener("submit", async (e) => {
  ...
  if (searchInput.value.length) {
    const fetchURL = await fetch(
      `${keyword_http}part=snippet&maxResults=20&q=${searchInput.value}&key=${API_KEY}`
    );
    const data = await fetchURL.json();
...
formObjRes.addEventListener("submit", async (e) => {
  ...
  if (searchInputRes.value.length) {
    const fetchURL = await fetch(
      `${keyword_http}part=snippet&maxResults=20&q=${searchInputRes.value}&key=${API_KEY}`
    );
    const data = await fetchURL.json();
...
async function watchVideo(id) {
  ...
  const fetchURL = await fetch(
    `${video_http}part=snippet%2CcontentDetails%2Cstatistics&id=${id}&key=${API_KEY}`
  );
  const data = await fetchURL.json();
  ...
  let description = videoData.snippet.description
    ...
    .slice(0, 200)
    .join("");
  let rest = videoData.snippet.description
    .split("")
    .slice(200, lengthNum)
    .join("");
  ...
  stats.innerHTML = `${
    statsCount[0]
  } views <small class="text-muted">${videoData.snippet.publishedAt
    .split("")
    .slice(0, 10)
    .join(
      ""
    )}</small>  <br/> <i class="fa-regular fa-thumbs-up" style="color: #0f0f10;"></i> ${
    statsCount[2]
  } Comments ${statsCount[1]}
  `;
...
async function backBtnFunc() {
  ...
  const fetchURL = await fetch(
    `${keyword_http}part=snippet&maxResults=20&q=${recent}&key=${API_KEY}`
  );
  const data = await fetchURL.json();
  filterFunc();
  videoCardContainer.innerHTML = "";
...
async function getChannelDetails(id) {
  ...
  // channel details
  const fetchURL = await fetch(
    `${channel_http}part=snippet%2CcontentDetails%2Cstatistics&id=${id}&key=${API_KEY}`
  );
  const data = await fetchURL.json();
  const videoData = data.items[0];

  // channel playlist
  const playlistURL = await fetch(
  ...
  backBtn2.innerHTML =
    '<i class="fa-solid fa-arrow-left fa-lg" style="color: #111213;"></i>';
  ...
  if (activityCard.length > 0) {
    ...
    <small>${activity.snippet.publishedAt
      .split("")
      .slice(0, 10)
      .join("")} Upload</small>`;
      cardDiv.append(thumbnail, cardBody);
...
<div class="row g-0">
  <div class="col-md-4">
      ...
      }" class="img-fluid rounded-start" alt="${videoData.snippet.title}">
  </div>
  <div class="col-md-8">
      <div class="card-body">
        <h5 class="card-title">${videoData.snippet.title}</h5>
        <p class="card-text">${videoData.snippet.customUrl}</p>
        <p class="card-text"><small class="text-muted">Since ${videoData.snippet.publishedAt
          .split("")
          .slice(0, 10)
          .join("")}</small></p>
      </div>
